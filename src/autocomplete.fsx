            ...
            let action (s : string) = 
                ...
                if split.Length > 1 then
                    try
                        ...
                        if o.Kind = "errors" then Globals.atom.emit("FSharp.Atom:Highlight", o.Data)
                    with
                    | ex -> ()
                cb s
...
type Autocomplete() = 
    ...
    member x.activate(state:obj) =     
        ...
        Globals.atom.on("FSharp.Atom:Highlight", unbox<Function>(HighlighterHandler.handle))
        Globals.atom.on("FSharp.Atom:Highlight", unbox<Function>(Views.ErrorPanelView.handle))
        ()
